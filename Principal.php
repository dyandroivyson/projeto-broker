...
{    
    ...
    {
        $this->escreveMensagem('Iniciando processamento');
        ...
        foreach ($arquivos as $arquivo) {
            ...
            $this->escreveMensagem('Lendo arquivo ' . $arquivo);
        ...
        $this->escreveMensagem('Validando regras');
        ...
        foreach ($linhas as $linha) {
            ...
            // Verificando celular
            if (!$this->checaCelular($linha['CELULAR'])) {
                $this->escreveMensagem(
            ...
            if (!$this->checaBlacklist($linha['DDD'], $linha['CELULAR'])) {
                $this->escreveMensagem(
            ...
            if (!$this->checaHorarioAgendamento($linha['HORARIO'])) {
                $this->escreveMensagem(
            ...
            if (!$this->checaMensagem($linha['MENSAGEM'])) {
                $this->escreveMensagem(
                    'Mensagem ' . $linha['IDMENSAGEM'] 
        ...
        $this->escreveMensagem('Ordenando mensagens');

        // Sanitarizando as mensagens
        $mensagens = $this->ordenaUnificaMensagens($mensagens_aptas);

        $this->escreveMensagem('Convertendo mensagens');

        // Convertendo para o formato final
        $mensagens_tratadas = $this->converteMensagens($mensagens);
        ...
        $this->escreveMensagem('Gerando arquivo no diretorio de saida ' . $arquivo_saida);
        ...
        $this->escreveMensagem('Processamento finalizado');
    }
     ...
     * @return void
     */
    public function escreveMensagem($mensagem)
    {
